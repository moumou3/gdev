...
static inline void tvsub(struct timespec *start, struct timespec *end, struct timespec *ret)
{
...
int gdev_test_matrixadd(uint32_t *a, uint32_t *b, uint32_t *c, int n)
{
	...
	tvsub(&ts1, &ts2, &ts);
	interval = ts.tv_sec * 1000 + ts.tv_nsec / 1000000;
